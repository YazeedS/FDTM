...
def construct_table():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template('table.html',  tables=[HTML(df_show.to_html(classes='table table-striped'))], titles=df.columns.values,
...
# @app.route("/table", methods=["post"])
# def construct_table_cb():
#
#     if request.method == "POST":
#         data_string = request.form["data"]
#     #delete all spaces , seperate columns, delete anything that is not numbers or commas
#
#     data_string = re.findall(r"\d+", data_string)
#
#     data = [float(x) for x in data_string]
#
#     n = len(data)
#     x_small = min(data)
#     x_large = max(data)
